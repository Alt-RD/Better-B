...
[lMod_HiveBody lMod_HiveBody_faces] = HT_Model_Conduction1D(         ... % Build a two layers 1D heat conduction problem
                          ...
                          'material', lHiveMat.externalAir,             ... % Air material for the hive body parts
...
% Faces are not in the following order: XM, XP, YM, YP based on global axis
...
lMod_SideWall1_facesCell = cell(6,1);   % Backup the faces of sidewalls for later use
lMod_SideWall2_facesCell = cell(6,1);   % Backup the faces of sidewalls for later use
...
for i=1:4
  ...
  % Create a thermal connection between sidewall 1 and sidewall 2
  lMod_Tmp = HT_Model_ConnectFaces(...
  ...
  lMod_Tmp = HT_Model_Connect(sprintf('mcSW1_ExtAir_%s', FaceDIRstrL{i}), ...
                                    lAirExtNode, ...          ... % Node of outside air
                                    lMod_SideWall1_faces(FaceXP),       ... % External face are connected to air
...
% Connect bottom sidewalls to the hive body 1
lMod_Tmp = HT_Model_ConnectFaces(...
  'mcHiveBody1WallBot', ...
...
lMod_Tmp = HT_Model_Connect(      'mcSW1_ExtAir_ZM', ...
                                  lAirExtNode, ...                 ... % Node of outside air
                                  lMod_SideWallBot_faces(FaceZM),  ... % External face are connected to air
                                  'g', lConvection.walls1_ext{5},  ... % Convection coefficient specified by user
                                  lOptions);
lModel = HT_Model_Merge(lModel, lMod_Tmp, lOptions);
...
% ============================= Underroof layer =================================
% Build the model for the air layer below the roof and upper the overframe
...
% =============================== Roof sides ===================================
lRoofSideAxis = { FaceDIRmat(:, [FaceYP,FaceZM,FaceXM]), ...
...
for i=1:4
  ...
  lMod_Tmp = HT_Model_ConnectFaces(...
    ...
    lMod_Roof_faces{i},                                 ...
    struct( 'uv1axis', lRoofSideAxis{i}(:,[X Z]),       ...
            'uv2axis', lRoofSideAxis{i}(:,[X Y])),      ...
    lOptions);
  lModel = HT_Model_Merge(lModel, lMod_Tmp, lOptions);

  % Connect the external roof side to the external air
  lRoofSide_extFace = HT_Face_Find(lMod_RoofSide_faces, 'orientation', FaceDIRmat(:,i), 'numberCheck', 1, 'returnType', true);
  lMod_Tmp = HT_Model_Connect(sprintf('mcRoofSide_ExtAir_%s', FaceDIRstrL{i}), ...
                                  lAirExtNode, ...                    ... % Node of outside air
                                  lRoofSide_extFace,       ... % External face are connected to air
                                  'g', lConvection.roof_ext{i},     ... % Convection coefficient specified by user
                                  lOptions);
  lModel = HT_Model_Merge(lModel, lMod_Tmp, lOptions);
